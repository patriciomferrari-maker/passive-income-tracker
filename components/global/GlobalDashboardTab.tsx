...
export function GlobalDashboardTab() {
    ...
    const compositionData = showValues ? stats.composition : [{ name: 'Oculto', value: 1, fill: '#1e293b' }];
    const projectedData = showValues ? stats.projected : [];
    const debtData = showValues ? stats.debtDetails : [];
                    ...
                    <CardContent className="h-[350px]">
                        <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={historyData} margin={{ top: 20, right: 10, left: 10, bottom: 0 }}>
                                ...
                                />}
                                {showValues && <Legend />}
                                <Bar dataKey="ON" stackId="a" fill="#0ea5e9" name="ONs" />
