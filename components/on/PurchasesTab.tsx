...
export function PurchasesTab({ market = 'ARG' }: { market?: string }) {
    ...
    useEffect(() => {
        Promise.all([
            ...
            fetch(`/api/investments/positions?type=${market === 'US' ? 'TREASURY,ETF,STOCK' : 'ON,CORPORATE_BOND,ETF,CEDEAR'}&market=${market}&t=${Date.now()}`, { cache: 'no-store' }).then(res => res.json())
